        ...
        response.data.calendars.forEach((calendar) => {
          if (calendar.startAt || calendar.start_at) {
            ...
            appsMap[dateKey].push({
              id: calendar.id,
              time: time,
              title: calendar.title || (eventType === 1 ? 'Phỏng vấn ứng viên' : eventType === 2 ? 'Nyusha' : 'Lịch hẹn'),
              candidate: calendar.jobApplication?.name || calendar.jobApplication?.cv?.fullName || 'N/A',
              job: calendar.jobApplication?.job?.title || 'N/A',
              location: calendar.description || 'Phòng họp',
