...
const AdminJobDetailPage = () => {
  ...
  const [hoveredCollapsibleCardIndex, setHoveredCollapsibleCardIndex] = useState(null);
  const [hoveredCollapsibleCardButton, setHoveredCollapsibleCardButton] = useState(null);
  const [hoveredSuggestCandidateButton, setHoveredSuggestCandidateButton] = useState(false);
  const [hoveredCopyUrlButton, setHoveredCopyUrlButton] = useState(false);
  ...
  const CollapsibleCard = ({ 
    ...
    const cardIndex = sectionKey;
    
    return (
      <div 
        className={`rounded-xl shadow-sm transition-shadow ${className}`}
        style={{ 
          backgroundColor: 'white', 
          borderColor: '#e5e7eb',
          borderWidth: '1px',
          borderStyle: 'solid',
          boxShadow: hoveredCollapsibleCardIndex === cardIndex ? '0 4px 6px -1px rgba(0, 0, 0, 0.1)' : '0 1px 2px 0 rgba(0, 0, 0, 0.05)'
        }}
        onMouseEnter={() => setHoveredCollapsibleCardIndex(cardIndex)}
        onMouseLeave={() => setHoveredCollapsibleCardIndex(null)}
      >
        <button
          onClick={() => toggleSection(sectionKey)}
          onMouseEnter={() => setHoveredCollapsibleCardButton(cardIndex)}
          onMouseLeave={() => setHoveredCollapsibleCardButton(null)}
          className="w-full flex items-center justify-between p-5 sm:p-6 transition-colors rounded-t-xl"
          style={{
            backgroundColor: hoveredCollapsibleCardButton === cardIndex ? '#f9fafb' : 'transparent'
          }}
        >
    ...
    <>
      ...
      <div className="flex-1 overflow-y-auto min-w-0 custom-scrollbar">
        <div className="w-full max-w-none space-y-5">
          ...
          <div className="rounded-xl shadow-sm p-5 sm:p-6" style={{ backgroundColor: 'white', borderColor: '#e5e7eb', borderWidth: '1px', borderStyle: 'solid' }}>
            <div className="flex items-center justify-between mb-5">
              <button
                ...
                className="flex items-center gap-2 transition-colors text-sm sm:text-base group"
                style={{
          ...
          {/* Orange Notice Box */}
          <div className="border-2 rounded-xl p-4 sm:p-5 flex items-start gap-3 shadow-sm" style={{ backgroundColor: '#fff7ed', borderColor: '#fdba74' }}>
            <AlertCircle className="w-4 h-4 sm:w-5 sm:h-5 flex-shrink-0 mt-0.5" style={{ color: '#ea580c' }} />
            <div className="text-xs sm:text-sm" style={{ color: '#7c2d12' }}>
      ...
      <div className="w-full lg:w-64 xl:w-72 flex-shrink-0">
        <div className="sticky top-4 lg:top-6">
          ...
          <div className="space-y-3">
            ...
            <button
              onClick={handleApply}
              onMouseEnter={() => setHoveredSuggestCandidateButton(true)}
              onMouseLeave={() => setHoveredSuggestCandidateButton(false)}
              className="w-full py-3 px-4 rounded-xl transition-all duration-200 font-semibold text-xs sm:text-sm flex items-center justify-center gap-2 shadow-md"
              style={{
                backgroundColor: hoveredSuggestCandidateButton ? '#facc15' : '#fbbf24',
                color: '#111827',
                boxShadow: hoveredSuggestCandidateButton ? '0 10px 15px -3px rgba(0, 0, 0, 0.1)' : '0 4px 6px -1px rgba(0, 0, 0, 0.1)'
              }}
            >
            ...
            <button
              ...
              onMouseEnter={() => setHoveredCopyUrlButton(true)}
              onMouseLeave={() => setHoveredCopyUrlButton(false)}
              className="w-full border-2 py-3 px-4 rounded-xl transition-all duration-200 font-semibold text-xs sm:text-sm flex items-center justify-center gap-2 shadow-sm"
              style={{
                borderColor: '#60a5fa',
                backgroundColor: hoveredCopyUrlButton ? '#eff6ff' : 'white',
                color: '#2563eb',
                boxShadow: hoveredCopyUrlButton ? '0 4px 6px -1px rgba(0, 0, 0, 0.1)' : '0 1px 2px 0 rgba(0, 0, 0, 0.05)'
              }}
            >
